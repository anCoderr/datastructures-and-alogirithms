...
class SegmentTree:
    ...
    def sum_query(self, a: int, b: int) -> int:
        return self.sum_query_utility(0, a, b, 0, self.n - 1)

    def sum_query_utility(self, index: int, a: int, b: int, left: int, right: int) -> int:
        ...
        left_child_index = self.get_left_child_index(index)
        right_child_index = self.get_right_child_index(index)
        sum_left = self.sum_query_utility(left_child_index, a, b, left, mid)
        sum_right = self.sum_query_utility(right_child_index, a, b, mid + 1, right)
